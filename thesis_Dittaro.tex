...
Il primo step è quello di modificare un'impostazione nel file di configurazione \textit{config.toml}, il campo \textbf{baseURL} va infatti modificato sostituendo il precedente contenuto con
\begin{center}
    \textit{"https://NOME\_UTENTE.github.io/NOME\_REPOSITORY/"}
\end{center}
Questo campo risulta importante perchè è quello utilizzato da Hugo per generare i link delle pagine staticamente. I link delle altre pagine vengono cotruiti a partire dal baseURL al quale viene aggiunto il nome specifico (ad esempio \textit{"https://NOME\_UTENTE.github.io/NOME\_REPOSITORY/people/"}).
\newline
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item git push
    \end{itemize}
    \item Esecuzione del comando \textbf{git subtree push - -prefix public origin gh-pages}: questo permette le creazione del branch \textit{gh-pages}, sul quale viene inserito solamente il contenuto della cartella "puiblic" 
\end{enumerate}
...
\end{enumerate}
La procedura di aggiornamento è visibile nella sezione \textit{Actions} della propria repository. Le operazioni "pages build and deployment" inizialmente avranno un simbolo giallo a simboleggiare lo svolgimento dell'operazione, 
...
Riepigolando, le operazioni da eseguire per la pubblicazione e l'aggiornamento sono le seguenti: 
\begin{itemize}
...
La barra di ricerca posta all'estremità destra del menù orizzonatale non viene inizialmente inclusa nel menù ad "hamburger". 
...
\newline
Navigando sul web, si possono trovare vari metodi per l'organizzazione del contenuto di questa pagina. Il metodo scelto nel caso di studio è quello raffigurato in .
...
\begin{itemize}
    \item \textbf{list.html}: contenuto all'interno della cartella \textit{themes/tella/layouts/people}, si occupa di organizzare il contenuto mostrato in .
    \newline
    ...
    Proprio quest'ultimo costrutto viene subito utilizzato per verificare la presenza di un'immagine nel fron-matter del corrispondente file markdown, in caso positivo viene posizionata la foto specificata altrimenti un'immagine di default. 
    Viene poi inserito il nome e poi riapplicato nuovamente il costrutto \textit{if} per verificare la presenza del campo "position" e del campo "e-mail", in caso negativo non viene mostrato nulla.  
\end{itemize} 
...
\newline \newline
Nel caso di riduzione dello schermo o dell'utlizzo di dispostivi diversi da un computer, la soluzione di porre tre membri per ogni riga non è più valida in quanto le sezioni si sovrappongono e diventano indistinguibili.
...
\newline  
Le dimensione generiche di un tablet sono comprese tra 768px e 1265px, si è scelto quindi di passare da una griglia di tre elementi ad una griglia di due elementi. Il codice corrispondente è il seguente:
\newline \newline
...
Nel caso di un cellulare, si riducono ulteriormente le dimensioni che generalmente sono comprese tra 320px e 767px, la griglia si riduce quindi ad un singolo elemento. Il codice è il seguente.
\newline \newline
...
In questo modo l'organizzazione della sezione \textit{people} sarà uniforme su qualsiasi dispositivo. I riusltato è visibile nelle \cref{img:computerp}, \cref{img:computert} e \cref{img:computertab}
...
Un'altra importante misura di un sito web è l'accessibilità, questa è definita come la capacità di un sito di essere utilizzato in modo efficace da persone con un diverso raggio di abilità e capacità.
...
\newline \newline
Nel caso di esempio riportato nelle prossime righe è stato utilizzando il browser "Microsoft Edge", attraverso gli strumenti di sviluppo è possibile infatti simulare diversi deficit visivi.  
